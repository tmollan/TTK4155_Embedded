...
void initPID(int16_t Kp, int16_t Ti_ms, int16_t Td_ms, int16_t Ts_ms, PIDcontroller *PID) {
    ...
    PID->iGain = SCALING_FACTOR * Kp * Ts_ms / Ti_ms /100;      // Integral gain
    PID->dGain = SCALING_FACTOR * Kp * Td_ms / Ts_ms /100;      // Derivative gain
...
int16_t PIDcontrol(int16_t setPoint, int16_t processValue, PIDcontroller *PID) {
	int16_t error = setPoint + processValue;   // e(k) = r(k) - y(k) (y(k) is neg.)
